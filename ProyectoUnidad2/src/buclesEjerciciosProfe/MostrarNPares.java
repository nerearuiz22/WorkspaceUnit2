...
public class MostrarNPares {
	...
	public static void main(String[] args) {
		...
		//We declare the variable number that starts at 0.
		int n = 0;
		...
		//For loop.
		//First the first condition is evaluated only once (int = 0;)
		...
		for ( int i=0; i<=2*(n-1); i+=2) {
			...
			if (i+2<=2*(n-1)) { //If the last even number, when subtracted by 1 and multiplied by 2, exceeds the value i added by 2, no comma is added since it is not the last number
				System.out.print (i );
			}else {
